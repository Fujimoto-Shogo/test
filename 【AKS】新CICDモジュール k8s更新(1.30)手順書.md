...
- [【AKS】新CICDモジュール k8s更新(1.30)手順書](#aks新cicdモジュール-k8s更新130手順書)
  ...
  - [事前作業](#事前作業)
  - [直前作業](#直前作業)
  - [AKS VersionUp (1.30)](#AKSVersionUp(1.30))
  - [LTS取り消し手順](#LTS取り消し手順)
...
* 更新後確認(1.27→1.30)
  * k8s version更新確認  
    ...
    ```
  * LTSが外れていないことの確認  
    下記コマンドを実行し、```"AKSLongTermSupport"```になっていることを確認
    ```
    az aks show --name aks-test20240610 -g aks-test20240610-resources-group --query supportPlan The behavior of this command has been altered by the following extension: aks-preview
    ```
    下記コマンドを実行し、```"Premium"```になっていることを確認
    ```
    az aks show --name aks-test20240610 -g aks-test20240610-resources-group --query sku.tier The behavior of this command has been altered by the following extension: aks-preview
    ```
  * k8s ノードversion更新確認  
